def Ingresaralumno():
    ...
    Notas = []
    while True:
        Nota = float(input("Ingrese la nota del alumno (ingrese un número negativo para terminar): "))
        if Nota < 0:
            break
        Notas.append(Nota)
    return Nombre, Notas
...
def main():
    Alumnos = {}
    while True:
        Cantidadalumnos = int(input("Ingrese la cantidad de alumnos que desea ingresar (ingrese un número negativo para terminar): "))
